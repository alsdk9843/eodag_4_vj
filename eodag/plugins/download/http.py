        ...
        def download_asset(asset: Asset, asset_stream: StreamResponse) -> None:
            ...
            if not asset["href"].startswith("file:"):
                self.set_statements(
                    asset,
                    {
                        "order:status": asset.product.properties.get(
                            "order:status", ""
                        ),
                        "eodag:order_id": asset.product.properties.get(
                            "eodag:order_id", ""
                        ),
                        "eodag:status_link": asset.product.properties.get(
                            "eodag:status_link", ""
                        ),
                        "eodag:search_link": asset.product.properties.get(
                            "eodag:search_link", ""
                        ),
                        "href": asset.get("href", ""),
